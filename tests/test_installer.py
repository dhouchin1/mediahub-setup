...
def test_render_compose_writes_caddyfile_before_compose_up(tmp_path):
    """Regression: the compose template bind-mounts config/caddy/Caddyfile
    into the caddy container, but the file was only generated by the *last*
    wiring task — long after `docker compose up`. Docker then created an
    empty directory at the mount source, Caddy failed to boot, and in local
    mode (Caddy publishes every service port) all health checks timed out."""
    settings = {
        **SAMPLE_SETTINGS,
        "enabled_services": ["caddy"],
        "caddy": {"domain": "mediahub.local", "mode": "local"},
    }
    installer.render_compose(tmp_path, settings)
    caddyfile = tmp_path / "config" / "caddy" / "Caddyfile"
    assert caddyfile.is_file()
    text = caddyfile.read_text()
    assert ":8989 {" in text  # sonarr site block, local mode
    assert "reverse_proxy qbittorrent:" in text


def test_render_compose_caddyfile_public_mode_and_gluetun_host(tmp_path):
    settings = {
        **SAMPLE_SETTINGS,
        "enabled_services": ["caddy", "gluetun"],
        "caddy": {"domain": "media.example.com", "mode": "public"},
    }
    installer.render_compose(tmp_path, settings)
    text = (tmp_path / "config" / "caddy" / "Caddyfile").read_text()
    assert "media.example.com" in text
    assert "reverse_proxy gluetun:" in text


def test_render_compose_without_caddy_writes_no_caddyfile(tmp_path):
    installer.render_compose(tmp_path, SAMPLE_SETTINGS)
    assert not (tmp_path / "config" / "caddy").exists()
